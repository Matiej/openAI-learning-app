...
@SpringBootTest
public class SystemMessagesTests extends BaseTest{
    ...
    void cityGuideTest() {
        ...
        SystemMessage systemMessage1 = new SystemMessage(systemPromptTemplate.createMessage().getContent());


        Message systemMessage = systemPromptTemplate.createMessage();

        PromptTemplate promptTemplate = new PromptTemplate("Tell me about New Orleans.");
        UserMessage userMessage = new UserMessage("Tell me about New Orleans.");
        Message userMessage2 = promptTemplate.createMessage();

        List<Message> messages = List.of(systemMessage, userMessage2);

        Prompt prompt = new Prompt(messages);
        System.out.println("System Messages just messages");
        System.out.println(openAiChatClient.call(prompt).getResult().getOutput().getContent());

        System.out.println("System Messages split for system and user messages");
        List<Message> messages2 = List.of(systemMessage1, userMessage2);
        Prompt prompt1 = new Prompt(messages2);
        System.out.println(openAiChatClient.call(prompt1).getResult().getOutput().getContent());
